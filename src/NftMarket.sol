...
contract NftMarket {
    ...
    error NftMarketplace_NotListed(address nftAddress, uint256 tokenId, address payToken);
    ...
    // 为了支持多种代币支付
    mapping(address => mapping(uint256 => mapping(address => uint256))) public nftList;
    ...
    function listItem(address token_address, uint256 tokenId, uint256 sale_price, address payToken) public {
        ...
        emit NftMarketplace_Listed(token_address, tokenId, sale_price, payToken);
    }
    ...
    function buy_with_signatures(
        ...
        _buynft(limitOrder.nft, limitOrder.tokenId, limitOrder.price, limitOrder.payToken);
    }
    ...
    function _buynft(
        ...
        if (payToken == address(0)) {
            ...
            IERC20(payToken).transferFrom(msg.sender, seller, price);
        }

        nft.safeTransferFrom(seller, msg.sender, tokenId);
        delete nftList[token_address][tokenId][payToken];

        emit NftMarketplace_Bought(token_address, tokenId, msg.sender, price, payToken);
    }
